...
const parseLargest = (data) => {
  const item = data
    .sort((a, b) => {
      const sizeA = a?.response?.content?.size || 0;
      const sizeB = b?.response?.content?.size || 0;
      return sizeA - sizeB;
    })
    .reverse()[0];
...
const parseLargestCompressed = (data) => {
  const item = data
    .sort(
      (a, b) => {
        const transferSizeA = a?.response?._transferSize || 0;
        const transferSizeB = b?.response?._transferSize || 0;
        const bytesInA = a?._bytesIn || 0;
        const bytesInB = b?._bytesIn || 0;
        return (transferSizeA + bytesInA) - (transferSizeB + bytesInB);
      }
    )
    .reverse()[0];
...
const parseLongest = (data) => {
  const item = data
    .sort((a, b) => {
      const timeA = a?.response?.time || 0;
      const timeB = b?.response?.time || 0;
      return timeA - timeB;
    })
    .reverse()[0];
...
const parseRequests = (data) => {
  ...
  return {
    ...
    bytesTransferred: sum(data.map((n) => n.response._transferSize || n._bytesIn)),
